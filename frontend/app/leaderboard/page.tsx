        ...
        const fetchLeaderboard = async () => {
            ...
            try {
                ...
                if (e.name === 'AbortError') {
                    setError('Request timed out. Server might be sleeping.')
                } else {
        ...
        <div className="min-h-screen bg-slate-50 text-slate-900 pb-24">
            ...
            <div className="max-w-6xl mx-auto pt-32 px-8">
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 bg-white rounded-[2.5rem] border border-red-100 shadow-xl shadow-red-900/5">
                        <div className="w-16 h-16 bg-red-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
                            <Trophy className="w-8 h-8 text-red-400" />
                        </div>
                        <h3 className="text-xl font-black text-slate-900 mb-2">Unavailable</h3>
                        <p className="text-slate-500 max-w-md mx-auto mb-6">{error}</p>
                        <button
                            onClick={() => window.location.reload()}
                            className="px-6 py-3 bg-[#000066] text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-900 transition-all"
                        >
                            Retry Connection
                        </button>
                    </div>
