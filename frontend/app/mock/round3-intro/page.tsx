...
import { MessageSquare, Lock, CheckCircle2, ChevronRight, Users } from 'lucide-react'

export default function Round3IntroPage() {
    const { session } = useAuth()
    const router = useRouter()
    const [loading, setLoading] = useState(true)
    const [passedRound2, setPassedRound2] = useState(false)
    ...
    if (loading) return (
        <div className="min-h-screen bg-white flex flex-col justify-center items-center gap-4">
            ...
            <p className="text-[#000066] font-black uppercase tracking-widest text-xs">Authenticating Clearance...</p>
        </div>
    )
            ...
            <div className="flex-1 max-w-4xl mx-auto w-full p-8 flex flex-col justify-center items-center">
                <div className="bg-white p-12 rounded-[3rem] shadow-2xl shadow-blue-900/5 border border-slate-100 text-center relative overflow-hidden">
                    ...
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-50 rounded-full border border-emerald-100 mb-8 items-center justify-center">
                        ...
                        <span className="text-emerald-600 font-black text-[10px] uppercase tracking-widest">Technical Phase Cleared</span>
                    </div>

                    <h1 className="text-5xl font-black text-slate-900 mb-6 tracking-tight leading-none uppercase">
                        Phase 03: <span className="text-[#000066]">Behavioral Evaluation</span>
                    </h1>

                    <p className="text-xl text-slate-500 mb-12 font-medium max-w-2xl mx-auto leading-relaxed">
                        Technical proficiency verified. You are now authorized to proceed to the AI-driven HR Proficiency Round.
                    </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left mb-12">
                        <div className="bg-slate-50 p-8 rounded-3xl border border-slate-100 group transition-all hover:bg-white hover:shadow-xl hover:shadow-blue-900/5">
                            <div className="w-12 h-12 bg-white rounded-2xl border border-slate-100 flex items-center justify-center mb-4 group-hover:bg-[#000066] transition-colors">
                                <Users className="w-6 h-6 text-[#000066] group-hover:text-white transition-colors" />
                            </div>
                            <h3 className="text-slate-900 font-black uppercase tracking-wider text-[10px] mb-2 tracking-[0.2em]">Objective</h3>
                            <p className="text-xs text-slate-500 font-bold uppercase tracking-tight">Soft skills, behavioral patterns, and cultural alignment with high-performance teams.</p>
                        </div>
                        <div className="bg-slate-50 p-8 rounded-3xl border border-slate-100 group transition-all hover:bg-white hover:shadow-xl hover:shadow-blue-900/5">
                            <div className="w-12 h-12 bg-white rounded-2xl border border-slate-100 flex items-center justify-center mb-4 group-hover:bg-[#000066] transition-colors">
                                <MessageSquare className="w-6 h-6 text-[#000066] group-hover:text-white transition-colors" />
                            </div>
                            <h3 className="text-slate-900 font-black uppercase tracking-wider text-[10px] mb-2 tracking-[0.2em]">Modality</h3>
                            <p className="text-xs text-slate-500 font-bold uppercase tracking-tight">Interactive AI dialogue synthesizing your responses for sentiment and confidence.</p>
                        </div>
                    </div>

                    <button
                        onClick={() => router.push('/mock/hr')}
                    ...
                    >
                        Initiate Phase 03 <ChevronRight className="w-6 h-6 group-hover:translate-x-1 transition-transform" />
                    </button>
                </div>
            </div>
        </div>
